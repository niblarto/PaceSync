...
import { loadConfirmedUniqueUris } from "@/lib/confirmed-unique-tracks";
...
// copy if that matching logic ever changes — and keep the
// confirmedUniqueUris exclusion in sync too: without it, this count kept
// including tracks already dismissed via "Keep this track" on the Settings
// page, so the dashboard banner could say "2 possible duplicates" forever
// while the Settings page's own (correctly filtered) list had genuinely
// nothing left to show — exactly why "review in Settings" looked broken.
...
export async function GET() {
  ...
  try {
    ...
    const confirmedUnique = loadConfirmedUniqueUris();
    const byKey = new Map<string, number>();
    for (const row of rows) {
      if (row.uri && confirmedUnique.has(row.uri)) continue;
