...
export async function POST(req: NextRequest) {
  ...
  const { title, segments, avoidUris, date, extraPlayCounts, strictPaceTolerance, segmentCandidateUris } = await req.json() as {
    title: string; segments: string[]; avoidUris?: string[]; date?: string; extraPlayCounts?: Record<string, number>;
    strictPaceTolerance?: boolean; segmentCandidateUris?: (string[] | null)[];
  };
  if (!segments?.length) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        }, mergedAvoidUris, extraPlayCounts, strictPaceTolerance, segmentCandidateUris);
        if (!result.ok) {
